...
import { PaymentsModal } from '../components/Payments';
...
const FEES: Partial<Record<DocType, number>> = { 'Transcript': 5000, 'CAC Certificate': 2500, 'Tax Clearance': 1500 };
...
export default function Verify() {
  const { docs, activeDocId, setActiveDocId, uploadDoc, runVerification, decideDoc, toast } = useApp();
  const [filter, setFilter] = useState<'all' | DocStatus>('all');
  const [query, setQuery] = useState('');
  const [dragOver, setDragOver] = useState(false);
  const [uploadForm, setUploadForm] = useState({ docType: 'National ID' as DocType, owner: '', branchId: 'br-victoria' });
  const [reviewNote, setReviewNote] = useState('');
  const fileRef = useRef<HTMLInputElement>(null);

  // Payment State
  const [payModal, setPayModal] = useState<{ open: boolean, amount: number, pendingDocId?: string, docType?: string }>({ open: false, amount: 0 });
  ...
  function handleFiles(files: FileList | null) {
    ...
    const f = files[0];
    const rec = uploadDoc(f.name, Math.max(120, Math.round(f.size / 1024)), uploadForm.docType, uploadForm.owner || 'Walk-in Customer', uploadForm.branchId);
    toast({ title: 'Document uploaded', desc: `${f.name} queued for verification`, tone: 'success' });
    
    const fee = FEES[uploadForm.docType];
    if (fee) {
      setPayModal({ open: true, amount: fee, pendingDocId: rec.id, docType: uploadForm.docType });
    } else {
      setTimeout(() => runVerification(rec.id), 600);
    }
  }

  function handlePaymentSuccess() {
    setPayModal(prev => ({ ...prev, open: false }));
    if (payModal.pendingDocId) {
      setTimeout(() => runVerification(payModal.pendingDocId!), 600);
    }
  }
    ...
    <div className="min-h-screen bg-paper pb-20 pt-[88px]">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-violet-600">Document verification</p>
            <h1 className="mt-2 font-display text-3xl font-extrabold tracking-tight text-ink-900 sm:text-4xl">Verify from home, arrive pre-cleared</h1>
            <p className="mt-2 max-w-xl text-sm text-slate-500">Upload IDs, transcripts and statements. Our checks scan for forgery, tampering and issuer validity — most clear in under a minute.</p>
          </div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[380px_1fr]">
          ...
          <div className="space-y-4">
            <div className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200/70">
              <p className="flex items-center gap-2 text-sm font-bold text-ink-900"><CloudUpload className="h-4 w-4 text-violet-600" /> New verification</p>
              <div className="mt-3 grid grid-cols-2 gap-2.5">
                <label className="col-span-2 text-xs font-bold text-slate-600">Document type
                  <select value={uploadForm.docType} onChange={e => setUploadForm({ ...uploadForm, docType: e.target.value as DocType })}
                    className="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-[13px] font-semibold outline-none focus:border-violet-400">
                    {DOC_TYPES.map(t => <option key={t}>{t}</option>)}
                  </select>
                </label>
                <label className="text-xs font-bold text-slate-600">Owner name
                  <input value={uploadForm.owner} onChange={e => setUploadForm({ ...uploadForm, owner: e.target.value })} placeholder="Full name"
                    className="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-[13px] font-semibold outline-none focus:border-violet-400" />
                </label>
                <label className="text-xs font-bold text-slate-600">Branch
                  <select value={uploadForm.branchId} onChange={e => setUploadForm({ ...uploadForm, branchId: e.target.value })}
                    className="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-[13px] font-semibold outline-none focus:border-violet-400">
              ...
              <div
                ...
                className={cn('mt-3 cursor-pointer rounded-2xl border-2 border-dashed p-6 text-center transition',
                  dragOver ? 'border-violet-500 bg-violet-50' : 'border-slate-200 bg-slate-50/60 hover:border-violet-300 hover:bg-violet-50/50')}>
                <input ref={fileRef} type="file" className="hidden" accept=".pdf,.jpg,.jpeg,.png" onChange={e => handleFiles(e.target.files)} />
                <ScanLine className="mx-auto h-8 w-8 text-violet-500" />
                <p className="mt-2 text-[13px] font-bold text-ink-900">Drop a file or click to browse</p>
                <p className="mt-0.5 text-[11px] text-slate-400">PDF, JPG or PNG · max 10 MB</p>
              </div>
            </div>
      ...
      <PaymentsModal 
        ...
        docType={payModal.docType || 'Document'} 
      />
    </div>
  );
}
